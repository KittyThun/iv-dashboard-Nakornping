...
st.markdown("""
    ...
    [data-testid="stExpander"] summary:hover { background-color: #bae6fd !important; }
        ...
        try:
            ...
            for _, row in df_notes.iterrows():
                ...
                if pd.notna(drug_name) and drug_name.lower() != 'nan':
                    notes[drug_name] = {}
                    if pd.notna(yellow_note) and yellow_note.lower() != 'nan': notes[drug_name]['yellow'] = yellow_note
                    if pd.notna(red_note) and red_note.lower() != 'nan': notes[drug_name]['red'] = red_note
...
st.markdown("""
    <div class="legend-box">
        ...
        <div class="legend-item"><div class="legend-color bg-yellow"></div><span class="legend-text"><b>🟡 สีเหลือง (Variable result):</b> ไม่แนะนำให้ใช้ กรณีจำเป็นต้องใช้ร่วมกัน ควรปรึกษาเภสัชกร</span></div>
...
if df is not None:
    ...
    drug_list = sorted(list(set([str(d) for d in df.index.tolist() + df.columns.tolist() if "compat" not in str(d).lower() and str(d).lower() != 'nan' and str(d) not in raw_fluids_to_exclude])))
    ...
    if check:
        ...
        if d5_sel != FLUID_PLACEHOLDER: selected_drugs.append(FLUID_MAPPING[d5_sel])
            ...
            for p1, p2 in pairs:
                ...
                def render_route(label, codes, drug_names, route_advice):
                    ...
                    mapping = {
                        ...
                        "Y": ("bg-green", "🟢 COMPATIBLE", ""), "C": ("bg-green", "🟢 COMPATIBLE", ""),
                        "ND": ("bg-gray", "⚪ NO DATA", "")
                    }
